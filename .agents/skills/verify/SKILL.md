...
For `games/<game>/<ver>/` and its web export `public/games/<game>/<ver>/`, read [Godot 游戏验证手册](references/godot-games.md). `games/verify.json` lists each version's checks; add new versions there, never special-case them in the runner.

1. **Machine checks first (shift-left).** `GODOT=/path/to/Godot_v4.4.1… npm run verify:games`: story exhaustive check, `--import`, headless `--storytest`/`--autotest` (must print `RESULT: … PASS`; any `ERROR`/`SCRIPT ERROR` line fails), re-export the Web `.pck` and require byte equality with the committed one, and `index.html` `fileSizes` equal to the committed `.pck`. Missing or wrong Godot version is FAIL, never SKIP. Report: `verification/games.json`. The Pages workflow runs it before browser acceptance and blocks deploy on failure. The story verifier peaks around 4.5 GB RAM; an OOM kill shows as a step with exit code null.
2. **Every fixed bug gets a regression check** in `--autotest` or `--storytest` that fails on the old code; record it in the version's `VERSION.md`.
3. **Test runs never touch player data.** Test modes write only `GS.TEST_SAVE_PATH`; a new test mode must be added to `TEST_FLAGS` in `game_state.gd`. Both test suites assert this first and `--autotest` refuses to wipe otherwise.
4. **Browser acceptance.** Run `python3 scripts/verify-game-web.py` against the production preview, and after merge against the Pages URL with `--sha <merge SHA>` once `build-info.json` shows that SHA. Exit 2 = BLOCKED (verifier's own network), rerun. Open the saved screenshots; pixel change alone is not content proof.
5. Keep separate: machine PASS, local browser PASS, public Pages PASS, gameplay depth, phone/touch, desktop builds, live-LLM character quality, and user art/story approval. Only claim what was run.
