...
## Games route (`games/` + `/games/`)

Complete games live in `games/<game>/<version>/` (Godot source) with their browser build in `public/games/<game>/<version>/`. `games/verify.json` lists each version's checks; add new versions there, do not special-case them in the runner.

1. **Machine checks first (shift-left).** Run `npm run verify:games` with Godot 4.4.1 (`GODOT=/path/to/Godot_v4.4.1…`; missing or wrong version is FAIL, never SKIP). Per version it runs the python story exhaustive check, `--import`, headless `--storytest` / `--autotest` (must print `RESULT: … PASS`; any `ERROR` / `SCRIPT ERROR` line fails the step), then re-exports the Web `.pck` and requires byte equality with the committed `public/games/…/index.pck`. Report: `verification/games.json`. The Pages workflow runs the same command before browser acceptance and blocks deploy on failure.
2. **Every fixed bug gets a regression check** in `--autotest` or `--storytest` that fails on the old code. Record the red→green evidence in the version's `VERSION.md`.
3. **Test runs must not touch player data.** `--autotest` / `--storytest` / `--storydemo` write only `user://momen_save_test.json`. If a new test mode is added, add its flag to `TEST_FLAGS` in `game_state.gd`.
4. **Source and shipped build stay in sync.** After changing scripts or data, re-export: `godot --headless --path games/<game>/<version> --export-pack "Web" public/games/<game>/<version>/index.pck`. After changing `data/viral_script.json` or `video/render_viral.py`, re-render the video and spot-check frames with ffmpeg (for example the CTA fade at 24.7 s must still show the dimmed arena).
5. **Browser acceptance.** Local: `npm run build && npm run preview`, open `/games/<game>/<version>/`, confirm the Godot engine boots (canvas renders, `index.wasm` served as `application/wasm`, no console errors), the title screen responds to a click, and 新手引导 advances past its first gate. Release: repeat on the public Pages URL and match live `build-info.json` `sourceSha` to the deployed commit.
6. Keep separate: machine PASS, local browser PASS, public Pages PASS, phone/touch, desktop builds (Win/mac), real-model (Haiku key) dialogue quality, and user approval. Only claim what was run.
