...
for (const g of manifest.games) {
  ...
  if (g.webPack) {
    ...
    // index.html 里的 GODOT_CONFIG.fileSizes 必须与已提交的 .pck 大小一致（只换 .pck 忘了改 html 会让加载进度失真）
    const html = join(committed, '..', 'index.html');
    const m = existsSync(html) && readFileSync(html, 'utf8').match(/"index\.pck":(\d+)/);
    const sizeOk = !!m && Number(m[1]) === readFileSync(committed).length;
    report.steps.push({name: `${g.path} html-filesizes`, status: sizeOk ? 'PASS' : 'FAIL',
      html: m ? Number(m[1]) : null, pck: existsSync(committed) ? readFileSync(committed).length : null});
    save();
    console.log(`[verify:games] ${g.path} html-filesizes: ${sizeOk ? 'PASS' : 'FAIL'}`);
    if (!sizeOk) {
      report.machine = 'FAIL'; save();
      console.error(`${html} 的 fileSizes["index.pck"] 与 ${committed} 实际大小不一致，请同步。`);
      process.exit(1);
    }
    if (!same) {
      report.machine = 'FAIL'; save();
