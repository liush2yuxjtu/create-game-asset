...
function step(name, command, args, cwd) {
  console.log(`\n[verify:games] ${name}`);
  const r = spawnSync(command, args, {cwd, encoding: 'utf8', timeout: 8 * 60 * 1000});
...
for (const g of manifest.games) {
  ...
  if (g.webPack) {
    ...
    // index.html 里的 fileSizes 供加载器使用，只重导 pck 时容易忘记同步
    const html = join(dirname(committed), 'index.html');
    const m = existsSync(html) && readFileSync(html, 'utf8').match(/"fileSizes":\{[^}]*"index\.pck":(\d+)/);
    const actual = statSync(committed).size;
    const sizeOk = !!m && Number(m[1]) === actual;
    report.steps.push({name: `${g.path} index.html-fileSizes`, status: sizeOk ? 'PASS' : 'FAIL', declared: m ? Number(m[1]) : null, actual});
    save();
    console.log(`[verify:games] ${g.path} index.html-fileSizes: ${sizeOk ? 'PASS' : 'FAIL'}`);
    if (!sizeOk) {
      report.machine = 'FAIL'; save();
      console.error(`${html} 的 fileSizes["index.pck"]=${m ? m[1] : '缺失'}，实际 ${actual}。重新 --export-release 或改正该数字。`);
      process.exit(1);
    }
  }
}
