...
import {catalog} from '../public/asset-lab/catalog.js';
import {AssetController,features} from '../public/asset-lab/controller.js';
const defaults=Object.fromEntries(Object.entries(features).map(([k,v])=>[k,v.default]));
// Recording boundary catches non-finite geometry and canvas state leaks across shared instances.
function recorder(){let depth=0;const calls=[];const gradient={addColorStop(...a){calls.push(['stop',...a]);}};const ctx=new Proxy({}, {set(obj,k,v){obj[k]=v;calls.push([k,typeof v==='object'?'gradient':v]);return true;},get(obj,k){if(k in obj)return obj[k];return (...args)=>{for(const n of args)if(typeof n==='number')assert.ok(Number.isFinite(n),k+' must receive finite geometry');if(k==='save')depth++;if(k==='restore')depth--;assert.ok(depth>=0);calls.push([k,...args]);if(k==='createRadialGradient')return gradient;};}});return {ctx,calls,depth:()=>depth};}
